...
import useRoutChange from "@/app/shared/hooks/useRouteChange";
...
const DashboardItems = ({ bottomContent }: { bottomContent?: boolean }) => {
    const { activeRoute, setActiveRoute } = useRoutChange();
            ...
            {!bottomContent ? (
                <>
                    {sideBarItems.map((item: DashboardSideBarTypes, index: number) => (
                        <Link key={index} href={item.url} className="p-2 flex items-center">
                            <span
                ...
                <>
                    ...
                    <p className="text-sm text-center pt-5 pb-10">
                        © 2024 Becodemy, Inc. All right reserved.
                    </p>
                </>
            )}
        </>
